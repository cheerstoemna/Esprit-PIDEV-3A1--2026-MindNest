    ...
    private List<LocalDateTime> findFreeSlots(
        ...
        while (cursor.isBefore(limit) && slots.size() < 5) {
            ...
            if (t.isBefore(LocalTime.of(8, 0))) {
                cursor = cursor.toLocalDate().atTime(8, 0);
                continue;
            }
            ...
            if (findPatientConflicts(userId, cursor, slotEnd, excludeId).isEmpty()
                    && findTherapistConflicts(therapistId, cursor, slotEnd, excludeId).isEmpty()) {
                slots.add(cursor);
            }
